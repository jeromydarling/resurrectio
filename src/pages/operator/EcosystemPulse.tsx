...
export default function EcosystemPulse() {
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-red-200">Ecosystem Pulse</h1>
        <p className="text-red-400/70 text-sm mt-1">Cross-ministry health and outcome metrics</p>
      </div>

      {/* Metric cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {metrics.map((m) => (
          <Card key={m.label} className="bg-red-950/40 border-red-900/40">
            <CardContent className="pt-6">
              ...
              <p className="text-2xl font-bold text-red-200">{m.value}</p>
              <p className="text-sm text-red-400/70 mt-1">{m.label}</p>
              {m.numValue !== null && (
                <Progress value={m.numValue} className="mt-3 h-1.5 bg-red-950/30 [&>div]:bg-red-600" />
              )}
            </CardContent>
          </Card>
        ))}
      ...
      <Card className="bg-red-950/40 border-red-900/40">
        <CardHeader>
          <CardTitle className="text-red-200 text-lg">Ministry Health Rankings</CardTitle>
          <CardDescription className="text-red-500/70">Ranked by composite health score</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          {ministryHealth.map((m, idx) => (
            <div key={m.name} className="flex items-center gap-4 p-3 rounded-lg bg-red-950/15 border border-red-900/15 hover:bg-red-950/25 transition-colors">
              <span className="text-lg font-bold text-red-600/60 w-8 text-center">{idx + 1}</span>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-red-200 truncate">{m.name}</p>
                <div className="flex gap-4 mt-1 text-xs text-red-500/60">
                  <span>Housing: {m.housing}%</span>
                  <span>Employment: {m.employment}%</span>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <Progress value={m.score} className="w-24 h-2 bg-red-950/30 [&>div]:bg-red-600" />
