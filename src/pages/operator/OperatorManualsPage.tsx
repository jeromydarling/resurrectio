 ...
 *
 * WHAT: Generates two downloadable PDFs: Resurrectio Field Guide (Tenant) and Operator Steward Manual.
...
function addCoverPage(doc: jsPDF, title: string, subtitle: string) {
  ...
  doc.text('Resurrectio™', 105, 100, { align: 'center' });
  doc.setFontSize(18);
  ...
  doc.text('From Incarceration to Restoration', 105, 268, { align: 'center' });
  doc.addPage();
}

// ── Manual 1: Resurrectio Field Guide ──

function generateFieldGuide(): jsPDF {
  const doc = new jsPDF();
  addCoverPage(doc, 'Resurrectio Field Guide', 'The Tenant Reference Book');
  let y = 25;
  ...
  if (y > 200) { doc.addPage(); y = 25; }
  y = addSection(doc, 'G. AI & Discernment in Resurrectio', y);
    ...
    try {
      if (manual === 'technical') {
        ...
        const filename = manual === 'field-guide'
          ? `Resurrectio-Field-Guide-${new Date().toISOString().split('T')[0]}.pdf`
          : `Resurrectio-Gardener-Steward-Manual-${new Date().toISOString().split('T')[0]}.pdf`;
        doc.save(filename);
      }
    } finally {
      setGenerating(null);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <Card className="border-border/60">
          <CardHeader>
            ...
            <CardDescription>
              The Platform Governance Reference — everything a Gardener needs to manage the entire Resurrectio platform.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="flex flex-wrap gap-1.5">
        ...
        <Card className="border-border/60">
          <CardHeader>
            ...
            <CardDescription>
              The Developer Reference — comprehensive technical documentation of the entire Resurrectio architecture.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="flex flex-wrap gap-1.5">
